...
function interpretLayouts(displayLayouts) {
  ...
  for(var i=0; i < displayLayouts.children.length; i++) {
    ...
    // console.log(layoutdata.children)
    var layout = {};
...
function parseTime(timestr) {
  ...
  d.setHours( parseInt(time[1]) + (time[5] ? 12 : 0) );
  d.setMinutes( parseInt(time[2]) || 0 );
  d.setSeconds( parseInt(time[4]) || 0 );
  var result = pad(d.getHours()) + ':' + pad(d.getMinutes()) + ':' + pad(d.getSeconds());
  return result;
}
